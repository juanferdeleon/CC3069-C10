...
__global__ void vadd (int *a, int *b, int *c, int N)
{
  ...
  __shared__ int s_in1[BLOCK_SIZE];
  __shared__ int s_in2[BLOCK_SIZE];
  s_in1[threadIdx.x] = a[myID];
  s_in2[threadIdx.x] = b[myID];
  
  if (myID < N) {
    c[myID] = s_in1[threadIdx.x] + s_in2[threadIdx.x];
  }
}
...
int main (void)
{
  ...
  printf("Tiempo que se tardo:  %e ms \n", time);
